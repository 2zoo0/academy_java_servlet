...
<head>
<meta charset="UTF-8">
<title>EL (6) 내장객체와 맵, 사용자 정으 ㅣ타입 객체의 사용 </title>
</head>
<body>
<%
...
<h3>EL 의 사용자 정으 객체(빈즈), 맵 객체에서 값 추출</h3>
<pre>
.(도트) 연산자로 값을 추출
...
1. Product 중 adidas 이름으로 request 에 추가된 객체를 추출
	...
	제품수량 : \${requestScope.adidas.quantity } = ${requestScope.adidas.quantity } 
...
2. reebok 이름이 requestScope에만 추가되어 있다면 객체 이름 생략 가능
	...
	제품수량 : \${reebok.quantity } = ${reebok.quantity } 
...
3. map 으로 추가된 객체를 추출
   (requestScope에만 추가되어 있다면 객체 이름 생략 가능)
	...
	제품수량 : \${adidasMap.S001.quantity } = ${adidasMap.S001.quantity } 
